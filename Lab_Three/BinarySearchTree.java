  ...
  BinarySearchTree<T extends Comparable<? super T>> {
    ...
    private BinaryNode<T> predecessor(BinaryNode<T> root, T entry){
      ...
      if(root != null){
        ...
        //replace true the condition in which we have to move right
          ...
          if(result == null){ //couldn't find a predecssor;
            ...
            //replace null with the first right parent (root)
            result = root;
          }
    ...
    private BinaryNode<T> successor(BinaryNode<T> root, T entry){
      ...
      if(root != null){
        ...
        if(compareResult == 0){ //found the node;
          ...
          //replace null with code to find the smallest in the right subtree
    ...
    private BinaryNode<T> findSmallest(BinaryNode<T> root){
      ...
      if(root != null){
        if(root.left != null){
          //replace null with code to find the smallest in the left subtree
