    ...
    private Node get(Node x, Integer key, int pos) {
        ...
        if ((x != null) && (pos >= 0)){
          if(x.key.equals(key)){
            ...
            result = get(x.left, key, pos-1); /* make a recursive call on the left child and the next bit */
          } else { //move right
            result = get(x.right, key, pos-1); /* make a recursive call on the right child and the next bit */
          }
        }
        return result;
    }
    ...
    private Node put(Node x, Integer key, Value val, int pos) {
        ...
        if (x == null){
          ...
          if(x.key.equals(key)){ //key is found, update value
            x.val = val;
            result = x;
            /* Change the value of the node x */
          } else if(bitAt(key, pos) == false){ //move left
            x.left = put(x.left, key, val, pos-1);/*make a recursive call on the left child and the next bit */
          } else { //move right
            x.right = put(x.right, key, val, pos-1); /* make a recursive call on the right child and the next bit */
          }
        }
        return result;
    }
