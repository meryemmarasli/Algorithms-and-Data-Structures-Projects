...
public class DLB<Value> {
    ...
    ****************************************************/
        ...
        if(x != null){
          if(x.letter == key.charAt(pos)){
            if(pos == key.length()-1){
              ...
              result = get(x.child, key, pos+1);/*Recurse on the child node*/;
            }
          } else {
            /*Recurse on the next sibling node*/
    ...
    private Node put(Node x, String key, Value val, int pos) {
        ...
        if (x == null){ //root case
            result = new Node();
            result.letter = key.charAt(pos); // setting root data to position
            result.level = pos; // setting level
            if(pos < key.length()-1){
              result.child = put(result.child, key, val, pos+1);/*Recurse on the child node*/;
            } else {
              result.val = val;
            }
